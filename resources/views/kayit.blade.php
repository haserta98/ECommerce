<!DOCTYPE html>
<html>

<head>

    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">

    <title>INSPINIA | Register</title>

    <link href="/kayitasset/css/bootstrap.min.css" rel="stylesheet">
    <link href="/kayitasset/font-awesome/css/font-awesome.css" rel="stylesheet">
    <link href="/kayitasset/css/plugins/iCheck/custom.css" rel="stylesheet">
    <link href="/kayitasset/css/animate.css" rel="stylesheet">
    <link href="/kayitasset/css/style.css" rel="stylesheet">

</head>

<body class="gray-bg">

<div class="middle-box text-center loginscreen   animated fadeInDown">
    <div>
        <div>

            <h1 class="logo-name">OT</h1>

        </div>
        <h3>Kayıt Ol</h3>

        <form class="m-t" method="post" role="form" action="{{route('kayit')}}">
            {{csrf_field()}}
            <div class="form-group">
                <input type="text" class="form-control" name="isim" placeholder="İsim" required="">
            </div>
            <div class="form-group">
                <input type="text" class="form-control" name="soyisim" placeholder="Soyisim" required="">
            </div>
            <div class="form-group">
                <input type="email" class="form-control" name="email" placeholder="Email" required="">
            </div>
            <div class="form-group">
                <input type="password" class="form-control" name="sifre" placeholder="Şifre" required="">
            </div>

            <div class="form-group">
                <input type="text" class="form-control" name="telefon" placeholder="Telefon" required="">
            </div>
            <div class="form-group">
                <textarea class="form-control" name="adres" placeholder="Adres"></textarea>
            </div>

            <div class="form-group">
                <div class="checkbox i-checks"><label> <input type="checkbox" name="sozlesme"><i></i> Sözleşmeyi ve şartları kabul ediyorum </label></div>
            </div>
            <button type="submit" class="btn btn-primary block full-width m-b">Kayıt</button>

            <p class="text-muted text-center"><small>Hesabınız var mı ?</small></p>
            <a class="btn btn-sm btn-white btn-block" href="/giris">Giriş</a>
        </form>

    </div>
</div>

<!-- Mainly scripts -->
<script src="/kayitasset/js/jquery-3.1.1.min.js"></script>
<script src="/kayitasset/js/bootstrap.min.js"></script>
<!-- iCheck -->
<script src="/kayitasset/js/plugins/iCheck/icheck.min.js"></script>
<script>
    $(document).ready(function(){
        $('.i-checks').iCheck({
            checkboxClass: 'icheckbox_square-green',
            radioClass: 'iradio_square-green',
        });
    });
</script>
</body>

</html>
